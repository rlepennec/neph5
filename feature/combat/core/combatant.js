...
export class NephilimCombatant extends Combatant {
    ...
    async setWound(wound, value, type) {
        ...
        const base = type === Constants.PHYSICAL ? actor.system.dommage.physique : actor.system.dommage.magique;
        const damages = duplicate(base);
        const isMortal = damages[Game.wounds.mortelle.id] != value;
        damages[wound.id] = value;
        await actor.update({ ['system.dommage.' + type]: damages });
        if (wound === Game.wounds.mortelle && type === Constants.PHYSICAL && isMortal) {
    ...
    getAllowedActionFromHistory(action) {
        ...
        // The action is not exclusive. Forbids to push this action if an exclusive action is already registered.
