...
export class FeatureBuilder {
    ...
    async dropOriginal(item) {
        switch (item?.type) {
            case 'appel':
                return Appel(this.actor, item, this.periode).dropOriginal(item);
            case 'atlanteide':
                return new Atlanteide(this.actor, item, this.periode).dropOriginal(item);
            case 'dracomachie':
                return new Dracomachie(this.actor, item, this.periode).dropOriginal(item);
            case 'formule':
                return new Formule(this.actor, null, this.periode).dropOriginal(item);
            case 'habitus':
                return new Habitus(this.actor, item, this.periode).dropOriginal(item);
            case 'invocation':
                return new Invocation(this.actor, item, this.periode).dropOriginal(item);
            case 'pratique':
                return new Pratique(this.actor, item, this.periode).dropOriginal(item);
            case 'rite':
                return new Rite(this.actor, item, this.periode).dropOriginal(item);
            case 'rituel':
                return new Rituel(this.actor, item, this.periode).dropOriginal(item);
            case 'sort':
                return new Sort(this.actor, item, this.periode).dropOriginal(item);
            case 'technique':
                return new Technique(this.actor, item, this.periode).dropOriginal(item);
            case 'tekhne':
                return new Tekhne(this.actor, item, this.periode).dropOriginal(item);
            default:
                return null;
        }
    }
